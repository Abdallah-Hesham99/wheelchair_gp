...
#include <math.h>
 double right_ticks_diff=0.0, left_ticks_diff=0.0;
unsigned long long  ppr = 2000*32; 
double wheel_diameter=0.3;
double center_dist = 0.61;
double distance_per_tick = (3.14*wheel_diameter)/ppr;
double TICKS_PER_METER=1/distance_per_tick;
double x,y,z,theta=0;
double vx=0,vy=0,w=0;
double old_time;
double cmd_linear = 0, cmd_angular=0;
...
void get_right_wheel_ticks(const std_msgs::Int32 right_ticks_msg)
{
...
void get_left_wheel_ticks(const std_msgs::Int32 left_ticks_msg)
{
...
void get_cmd(const geometry_msgs::Twist cmd)
{
...
//	double sl = distance_per_tick * left_ticks_diff ;
	...
	double dtheta = (sl-sr)/center_dist;
...
//	odom.pose.pose.orientation = odom_quat;
	...
	spd_msg.linear.x = vx;
	spd_msg.linear.z=cmd_linear;
	spd_msg.angular.z = w;
	spd_msg.angular.x = cmd_angular;
...
ros::Subscriber rsub = nd.subscribe("wheel_encoders/right_ticks",100,Calc_Right, ros::TransportHints().tcpNoDelay());
ros::Subscriber lsub = nd.subscribe("wheel_encoders/left_ticks",100,Calc_Left, ros::TransportHints().tcpNoDelay());
ros::Subscriber cmd_sub = nd.subscribe("/cmd_vel",1000,get_cmd);
ros::Publisher odom_pub = nd.advertise<nav_msgs::Odometry>("odom",5000);
ros::Publisher spd_pub = nd.advertise<geometry_msgs::Twist>("/control/current_speed",5000);
ros::Rate r(50);
